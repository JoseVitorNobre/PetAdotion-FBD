...
-- Consultas que respondem as perguntas feitas
...
SELECT P.id, P.nome, D.cpf, D.nome
...
SELECT TP.nome, COUNT(P.tipo_animal)
...
-- Colocar os outros que não aparecem como 0
SELECT TP.nome, COUNT(P.tipo_animal)
FROM tipo_animal TP JOIN pet P ON
...
SELECT DON.cpf, DON.nome, COUNT(DON.cpf)
FROM dono DON JOIN doacao DOA ON
     DON.cpf = DOA.cpf_doador
GROUP BY DON.cpf, DON.nome

-- Quantas adoções cada dono fez?
SELECT D.cpf, D.nome, COUNT(D.cpf)
FROM dono D JOIN adocao A ON
